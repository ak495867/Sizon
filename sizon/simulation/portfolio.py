...
def validate_order(order: Order, price: float, config: PortfolioConfig) -> list[str]:
    ...
    if order.order_type == "stop" and order.stop_price is None:
        errors.append("stop_price_required")
    if price <= 0:
        errors.append("invalid_market_price")
    return errors


def execute_order(
    order: Order, market_price: float, liquidity: float, config: PortfolioConfig
) -> Order:
    ...
    order.status = "filled" if fill == abs(order.quantity) else "partial"
    return order


def apply_risk_limits(
    target_position: float, equity: float, price: float, config: PortfolioConfig
) -> float:
    if equity <= 0:
        return 0.0
...
def liquidation_check(state: PortfolioState, config: PortfolioConfig) -> bool:
    if state.equity <= 0 or (
        state.margin_used > 0
        and state.equity / state.margin_used < config.liquidation_threshold
    ):
