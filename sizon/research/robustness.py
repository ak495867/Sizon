...
def cost_sensitivity(
    feed: DataFeed, genome: Node, execution: ExecutionModel | None = None
):
    base = execution or ExecutionModel()
    results = []
    for multiplier in (0.5, 1.0, 1.5, 2.0, 3.0):
        e = replace(
            ...
            impact_bps_per_turnover=base.impact_bps_per_turnover * multiplier,
        )
        results.append(
            {
                "cost_multiplier": multiplier,
                "execution": e.as_dict(),
                "metrics": run_backtest(feed, genome, e).metrics(),
            }
        )
...
def _perturb(node: Node, delta: int) -> Node:
    if isinstance(node, Primitive):
        return Primitive(node.name, max(2, node.period + delta))
    if isinstance(node, Binary):
        return Binary(node.op, _perturb(node.left, delta), _perturb(node.right, delta))
    return node
...
def parameter_sensitivity(
    ...
    return {
        ...
        "scenarios": [
            {
                "delta": d,
                "metrics": run_backtest(feed, _perturb(genome, d), execution).metrics(),
            }
            for d in deltas
        ],
    }
...
def monte_carlo_returns(
    ...
    result = run_backtest(feed, genome, execution)
    rng = random.Random(seed)
    source = result.returns[1:]
    samples = []
    if not source:
        return {
            "method": "monte_carlo_trade_reshuffle",
            "simulations": 0,
            "quantiles": {},
        }
    ...
    def q(key, p):
        values = sorted(x[key] for x in samples)
    ...
    return {
        ...
        "quantiles": {
            "total_return": {
                "p05": q("total_return", 0.05),
                "p50": q("total_return", 0.5),
                "p95": q("total_return", 0.95),
            },
            "sharpe": {
                "p05": q("sharpe", 0.05),
                "p50": q("sharpe", 0.5),
...
def robustness_suite(
    ...
    return {
        "cost_sensitivity": cost_sensitivity(feed, genome, execution),
        "parameter_sensitivity": parameter_sensitivity(feed, genome, execution),
        "monte_carlo": monte_carlo_returns(feed, genome, execution, simulations, seed),
    }
