...
def _feed(rows, source):
    return DataFeed(list(rows), source)


def _aggregate(folds):
    if not folds:
        return {"folds": 0, "metrics": {}}
    keys = folds[0].metrics
    return {
        "folds": len(folds),
        "metrics": {k: sum(f.metrics[k] for f in folds) / len(folds) for k in keys},
    }
...
def walk_forward(
    ...
    initial = max(1, int(n * train_size))
    folds = []
    for fold in range(n_splits):
        test_start = initial + fold * test_n
        test_end = min(n, test_start + test_n)
        if test_start >= n or test_end <= test_start:
            break
        train_start = 0 if expanding else max(0, test_start - initial)
        train_end = test_start
        test = _feed(feed.rows[test_start:test_end], feed.source)
        metrics = run_backtest(test, genome, execution).metrics()
        folds.append(
            FoldResult(fold, train_start, train_end, test_start, test_end, metrics)
        )
    return {
        **_aggregate(folds),
        "method": "walk_forward",
        "leakage_control": "future test windows are never used in preceding train windows",
...
def purged_cross_validation(
    ...
    n = len(feed.rows)
    fold_n = max(1, n // n_splits)
    folds = []
    for fold in range(n_splits):
        test_start = fold * fold_n
        test_end = n if fold == n_splits - 1 else min(n, (fold + 1) * fold_n)
    ...
    return {
        **_aggregate(folds),
        "method": "purged_cross_validation",
