...
from sizon.core.expression import (
    Node,
    Primitive,
    Series,
    Constant,
...
def check(node: Node):
    if isinstance(node, (Series, Constant)):
        return "series" if isinstance(node, Series) else "scalar"
    ...
    if isinstance(node, Binary):
        ...
        if left not in {"series", "scalar"} or right not in {"series", "scalar"}:
            raise TypeError("binary arithmetic requires scalar or series operands")
