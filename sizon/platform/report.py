...
def build_report(run_dir):
    ...
    rows = "\n".join(
        f"<tr><td>{html.escape(r['strategy_id'])}</td><td>{r['generation']}</td><td>{r['test_metrics'].get('sharpe',0):.4f}</td><td>{r['test_metrics'].get('max_drawdown',0):.4f}</td></tr>"
        for r in records
    )
    ...
    (run_dir / "report.html").write_text(content)
    (run_dir / "report.md").write_text(
        f"# Sizon Research Report\n\nStrategies evaluated: {len(records)}\n\n```json\n{json.dumps(summary,indent=2)}\n```\n"
    )
...
def build_top_report(run_dir, top_n=10):
    ...
    rows = "\n".join(
        f"<tr><td>{html.escape(str(r['strategy_id']))}</td><td>{html.escape(str(r['expression']))}</td><td>{r['test_sharpe']:.4f}</td><td>{r['test_drawdown']:.4f}</td><td>{r['robustness_score']:.2f}</td></tr>"
        for r in ranked
    )
    ...
    target = run_dir / "top_strategies.html"
